/*
 * Luau Members types and rendering functions.
*/
...
export type ReferenceMap = Map<string, { href: string; label: string }>;
...
const BADGES: Badges = {
    ...
    Client: {
        SVG: '<svg width="12" height="12" viewBox="0 0 337 304" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M0 201h337v36H0v-36zm0 0zM0 0v188h337V0H0zM218 287h30c12 0 12 17 0 17H88c-11 0-11-17 0-17h31l10-39h79l10 39z" /></svg>',
        Color: '#349AD5',
    ...
    Yields: {
        ...
        Color: '#f1c40f',
        Title: 'This is a yielding function. When called, it will pause the Lua thread that called the function until a result is ready to be returned, without interrupting other scripts.',
    },
...
export function renderMetaBadge(label: string) {
...
/// Is any member of a class deprecated?
function isDeprecated(member: { deprecated?: DeprecatedTag }) {
	return member.deprecated !== undefined;
}

/// Is a LuauFunction tagged with (custom tag) @signal
function isSignal(member: LuauFunction) {
	return member.tags?.some((tag) => tag.name.toLowerCase() === 'signal') ?? false;
}
...
/// Render a single member of a Class.
...
/// Renders the table used in the summary (inspired by Roblox's docs :) )
...
/// Render an entire Class to Markdown.
/// Refs are used to link up the types that are used in signatures and info.
export function renderClassMarkdown(luaClass: LuauClass, refs: ReferenceMap) {
	const lines: string[] = [];
	...
	const summaryEntries = [
		{ icon: ICONS.property, title: 'Properties', members: properties, kind: 'property' as const },
		{ icon: ICONS.function, title: 'Functions', members: functions, kind: 'function' as const },
		{ icon: ICONS.event, title: 'Events', members: signalFunctions, kind: 'function' as const },
		{ icon: ICONS.type, title: 'Types', members: types, kind: 'type' as const },
